...
prog_thresh=0.01,clip_s=False,s_min=1e-3,s_max=5,device=None,track_accept=False):
    """
      EagerPy implementation of Importance splitting estimator 
      Args:
         gen: generator of iid samples X_i, as EagerPy tensors        [fun]
...
prog_thresh=0.01,clip_s=False,s_min=1e-3,s_max=5,device=None,track_accept=False,allow_unfinished=False):
    """
      EagerPy batch implementation of Importance splitting estimator
      Args:
    ...
    while (tau_j<=tau).item():              #loose equality
        ...
        for _ in range(T):
            ...
            Z=ep.where(accept_flag,input=W,other=Z)
            
            SZ=ep.where(accept_flag,input=SW,other=SZ)
