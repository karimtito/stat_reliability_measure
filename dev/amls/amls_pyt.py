...
import torch
import math
from stat_reliability_measure.dev.torch_utils import normal_kernel
...
last_particle=False):
    """
      ...
      Args:
         ...
         h: score function on the gaussian latent space               [fun] 
    ...
    while (n<n_max) and (tau_j<tau).item():
        ...
        SZ = torch.zeros((N-K),device=device)
    ...
    # step E: Last round
    K_last = (SX>=tau).sum().item() # count the nb of h above the target threshold
...
track_s=False,track_levels=False, track_X=False):
    ...
    while (tau_j<=tau).item():              #loose equality
        n += 1 # increase iteration number
        if n >=n_max:
            if allow_unifinished:
                break
            else:
                raise RuntimeError(f'The estimator failed (n_max={n_max}). Increase n_max?')
        # step C: Keep K highest scores samples in Y
        Y = X[ind[0:K],:]
        SY = SX[ind[0:K]] # Keep their scores in SY
        # step D: refresh samples
        ...
        for _ in range(T):
            ...
            Z=torch.where(accept_flag[:,None],input=W,other=Z)
