...
import math
import numpy as np
from stat_reliability_measure.dev.torch_utils2 import TimeStepPyt, adapt_verlet_mcmc, adapt_verlet_mcmc2, apply_gaussian_kernel,verlet_mcmc
...
dt_min=1e-5,dt_max=1e-2,v_min_opt=True,kappa_opt=False,
 ...
 lambda_0=1,g_target=0.8,**kwargs):
    ...
    assert gaussian== True, "Only Gaussian (latent or explicit/prior) is managed by this method"
        ...
        if n==1:
            ...
            if GK_opt:
                ...
                if adapt_step:
                    ...
                    Y,v_y,grad_v_y,nb_calls,dict_out=verlet_mcmc(q=Y,beta=beta,gaussian=gaussian,
                                        V=V,gradV=gradV,T=T, L=L,kappa_opt=kappa_opt,delta_t=dt,device=device,save_H=track_H,save_func=None,
                                        scale_M=scale_M)
                    if track_H:
