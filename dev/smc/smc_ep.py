...
def SimpAdaptBetaPyt(beta_old,v,g_target,search_method=dichotomic_search_d,max_beta=1e6,verbose=0,multi_output=True,v_min_opt=False):
    ...
    """
    ess_beta = lambda beta : ep.exp(-(beta-beta_old)*(v)).mean() #decreasing funtion of the parameter beta
    if v_min_opt:
        v_min=v.min()
        ess_beta=lambda beta : ep.exp(-(beta-beta_old)*(v-v_min)).mean() 
    ...
    if multi_output:
        if v_min_opt:
            g=ep.exp((-(new_beta-beta_old)*v)).mean() #in case we use v_min_opt we need to output original g
        return (new_beta,g)
    else:
        return new_beta
...
def nextBetaESS(beta_old,v,ess_alpha,max_beta=1e6,verbose=0,thresh=1e-3,debug=False,eps_ess=1e-10):
    ...
    ESS = lambda beta: (ep.exp(-(beta-beta_old)*v).sum())**2/(ep.exp(-2*(beta-beta_old)*v).sum()+eps_ess)
    if debug:
 ...
 GK_opt=False,GV_opt=False,dt_d=1,skip_mh=False):
    ...
    while not reach_beta_inf:
        ...
        if n==1:
            ...
            dt= ep.clip(dt_scalar*X.ones(shape=(N,dt_d))+sig_dt*X.normal(shape=(N,dt_d)),min=dt_min,max=dt_max)
            ind_L=np.random.randint(low=L_min,high=L,size=(N,)) if L_min<L else L*X.ones(shape=(N,))
            if track_calls:
            ...
            if adapt_dt:
                ...
                if accept_rate>target_accept+accept_spread:
                    ...
                    dt*=dt_decay
                dt=ep.clip(dt,min=dt_min,max=dt_max)
                if verbose>=2.5:
        ...
        if reach_beta_inf or reach_rate>=min_rate:
            ...
            if v_min_opt:
                G_v_min= ep.exp(-(beta-beta_old)*(v-v.min()))
                to_renew=(G_v_min<U) 
            else:
                to_renew = (G<U)
        ...
        if nb_to_renew>0:
            ...
            X=ep.where(to_renew,X,X[to_renew.flatten()][renew_idx])
 ...
 GK_opt=False,GV_opt=False,dt_d=1,skip_mh=False):
    ...
    while not reach_beta_inf:
        ...
        if n==1:
            ...
            dt= ep.clip(dt_scalar*X.ones(shape=(N,dt_d))+sig_dt*X.normal(shape=(N,dt_d)),min=dt_min,max=dt_max)
            ind_L=np.random.randint(low=L_min,high=L,size=(N,)).float() if L_min<L else L*X.ones(shape=(N,))
            if track_calls:
                Count_v = 3*N #N calls to V + 2*N calls to grad_V
...
def SamplerSMC3(gen,  V, gradV,adapt_func,min_rate=0.8,alpha =0.1,N=300,T = 1,L=1,n_max=5000, 
max_beta=1e6, verbose=False,device=None,
track_accept=False,track_beta=False,return_log_p=False,gaussian=False,
adapt_dt=False,
track_calls=True,track_dt=False,track_H=False,track_v_means=False,track_ratios=False,
 ...
 GK_opt=False,GV_opt=False,dt_d=1,skip_mh=False):
    ...
    while not reach_beta_inf:
        ...
        if n==1:
            ...
            dt= ep.clip(dt_scalar*X.ones(shape=(N,dt_d))+sig_dt*torch.randn(size=(N,dt_d),device=device),min=dt_min,max=dt_max)
            ind_L=np.random.randint(low=L_min,high=L,size=(N,)).float() if L_min<L else L*X.ones(shape=(N,))
            if track_calls:
                Count_v = 3*N #N calls to V + 2*N calls to grad_V
