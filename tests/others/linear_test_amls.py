...
for p_t in config.p_range:
    ...
    def V_batch(X,c=c):
        return np.clip(c-X[:,0],a_min=0, a_max = np.inf)
    amls_gen = lambda N: np.random.normal(size=(N,d+2))
    batch_transform = lambda x: (x/LA.norm(x,axis=1)[:,None])[:d]
    normal_kernel =  lambda x,s : (x + s*np.random.normal(size = x.shape))/np.sqrt(1+s**2) #normal law kernel, appliable to vectors 
    h_V_batch= lambda x: -V_batch(batch_transform(x)).reshape((x.shape[0],1))
            ...
            for s in config.s_range:
                for ratio in config.ratio_range: 
                    ...
                    for i in tqdm(range(config.n_rep)):
                        t=time()
                        amls_res = amls.ImportanceSplitting(amls_gen, normal_kernel,K=K, N=N,s=s,  h=h_V_batch, 
