...
for l in inp_indices:
    ...
    for idx in range(len(config.epsilons)):
        ...
        V_ = lambda X: 0.5*t_u.V_pyt(X,x_0=x_0,model=model,low=low,high=high,target_class=y_0,gaussian_latent=config.gaussian_latent)
        gradV_ = lambda X: V_(X)[:,None]*t_u.gradV_pyt(X,x_0=x_0,model=model,low=low,high=high, target_class=y_0,gaussian_latent=config.gaussian_latent)
