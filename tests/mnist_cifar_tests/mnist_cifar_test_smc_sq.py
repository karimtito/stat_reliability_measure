...
parser.add_argument('--half_factor',type=str2bool,default=config.half_factor)
...
for l in inp_indices:
    ...
    for idx in range(len(config.epsilons)):
        ...
        V_ = lambda X: 0.5*t_u.V_pyt(X,x_clean=x_clean,model=model,low=low,high=high,target_class=y_clean,gaussian_latent=config.gaussian_latent)
        gradV_ = lambda X: V_(X)[:,None]*t_u.gradV_pyt(X,x_clean=x_clean,model=model,low=low,high=high, target_class=y_clean,gaussian_latent=config.gaussian_latent)
                ...
                for L in config.L_range:
                    for alpha in config.alpha_range:       
                        for N in config.N_range:
                            ...
                            "model_name":model_name,"image_idx":l, "sq":True,
