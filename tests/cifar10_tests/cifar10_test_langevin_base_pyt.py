...
if config.track_cpu:
    ...
    brand_keys =  [e for e in cpu_info.keys() if 'brand' in e]  
    if len(brand_keys)>0:
        brand_key=brand_keys[0]
        config.cpu_name=cpuinfo.get_cpu_info()[brand_key]
        ...
        for N in config.N_list: 
            for rho in config.rho_list:
                for alpha in config.alpha_list:
                    ...
                    log_name=log_name+'r'+float_to_file_float(rho)+'_'+'_'+loc_time
