...
import GPUtil
import argparse
from dev.torch_utils import project_ball_pyt, projected_langevin_kernel_pyt
...
class config:
    ...
    T_range=[1,5,10,20]
    min_rate=0.51
    rho_range=[80,100,120]
    alpha_range=[0.005,0.01,0.02]
...
parser.add_argument('--rho_range',type=str2floatList,default=config.rho)
...
nb_runs = config.n_rep*np.prod(param_lens)
            ...
            for alpha in config.alpha_range:       
                for N in config.N_range:
                    ...
                    for i in iterator:
                        ...
                        rho=rho,alpha=alpha,T=T,V= V,gradV=gradV,verbose=0,n_max=10000)
                        t1=time()-t
                        times.append(t1)
                        ests.append(p_est)
                    ...
                    'rho':rho,'alpha':alpha,'n_rep':config.n_rep,'min_rate':config.min_rate},ignore_index=True)
                    if run_nb%config.save_every==0:
                        results.to_csv('../logs/linear_tests/torch_results.csv',index=False)
