...
for l in inp_indices:
    ...
    for idx in range(len(config.epsilons)):
        ...
        score = lambda X: -t_u.V_pyt(X,x_0=x_0,model=model,low=low,high=high,target_class=y_0,gaussian_latent=config.gaussian_latent)
        for N in config.N_range: 
            for bs in config.b_range :
                ...
                results={'method':method_name,'gaussian_latent':str(config.gaussian_latent),'image_idx':l,
                    'epsilon':epsilon,"model_name":model_name,'n_rep':config.n_rep,
