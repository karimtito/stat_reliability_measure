...
import stat_reliability_measure.dev.amls.hyptest_amls_pyt import HypTestImportanceMLS
from stat_reliability_measure.dev.hmls.hmls_config import HMLS_Config
from stat_reliability_measure.dev.hmls.hyptest_hmls_pyt import HypTestHybridMLS
from stat_reliability_measure.config import ExpToyConfig
from itertools import product as cartesian_product

method_config_dict={'amls':MLS_TEST_Config,
                    'hmls':HMLS_TEST_Config,}

method_func_dict={'amls':amls_pyt.ImportanceSplittingPyt,
                  'hmls':HybridMLS,
                  }

def run_stat_test(method='amls_batch', p_target_range=[], score_name='linear', p_target=1e-3,
                  p_c= 1e-5, alpha_test=0.99,
                     noise_dist='gaussian',
    ...
    if len(results_df)>0:
        cert_rate = results_df['certs'].mean()
    else:
        cert_rate = None
