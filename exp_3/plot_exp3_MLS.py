...
import os
from stat_reliability_measure.dev.utils import float_to_file_float,path_to_stdlog10,path_to_meanlog10,get_sel_df
...
T_range = [10,]
triplets=[('input_index',img_idx,'='),('n_rep',50,'>='),
...
for T in T_range:
    csv_ams_T = get_sel_df(df=csv_res_webb, cols=['N','T'],vals=[20,T],conds=['>=','='],
                    triplets=triplets).sort_values(by=x_col)
    l = len(csv_ams_T)
    ...
    plt.errorbar(x = N_, y = csv_ams_T[y_col].values,yerr=csv_ams_T[std_col].values,label =f"{method_str}, T= {T}, alpha={ess_alpha}")
